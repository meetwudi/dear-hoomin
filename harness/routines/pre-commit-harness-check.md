...
- UX copy restraint: check `harness/development-principles.md` and `harness/routines/ux-copy-review.md` for changed user-facing UI. Remove redundant visible labels, duplicate headings, and helper text unless the copy changes a decision, prevents a likely mistake, or is required as the visible accessible name.
- UX style system discipline: check `harness/routines/ux-style-guide.md` for changed user-facing UI. Reuse existing UI primitives, semantic theme tokens, and narrow modifiers before adding bespoke one-off styling. Promote repeated colors, radii, shadows, spacing, or control sizing to reusable tokens instead of scattering literals.
...
- Unit test mock discipline: unit test mocks must be centralized in the touched app's unit-test support area, and each test must make it clear which boundaries are mocked before importing the subject under test. Do not scatter ad hoc mocks through production folders or hide broad mocks in individual tests when a shared support helper is more explicit.
...
- `UX copy restraint`: pass/fixed, with redundant labels/helper text removed or rationale for any visible explanatory copy.
- `UX style system discipline`: pass/fixed, with reused primitives/tokens noted or rationale for any new bespoke styling.
...
- `Unit test mock discipline`: pass/fixed, with touched unit-test support files and mocked boundaries noted or "no unit tests changed".
