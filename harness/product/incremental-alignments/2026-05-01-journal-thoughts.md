# Journal Musings Alignment

## Product Alignment

- Dear Hoomin is not a social product; hoomin-created entries are called journals, not posts.
- The timeline is made of entries. An entry can be a daily musing or a journal musing.
...
- A journal creates a pet musing that appears in the same today interface as the daily musing.
- The home page can show multiple entries for today: the daily musing plus any journal-created musings.
...
- Musing generation should be organized so different musing sources can share a clear generation path without hiding source-specific context.
- Daily musings use the pet, the hoomin's extra musing instructions, and recent musing text from the previous 30 days to reduce repetition.
- Journal musings use the pet, the hoomin's extra musing instructions, the journal note, and one uploaded photo to control token usage.
- Journal musing images use the selected avatar as the pet identity anchor and may use one uploaded journal photo as scene/context input.

## Mobile/API Alignment

- Treat timeline entry as a product and API concept even when the UI does not show that term.
- Entry payloads should have stable IDs, source kind, pet identity, local date, musing text, media ordered for display, and share targets for selected media.
...
- When Dear Hoomin supports multiple pets per family, hoomins need controls to add pets, archive pets while keeping musings, and delete pets with their musings.
- Multi-pet families need pet identification during photo upload so Dear Hoomin can classify which pet appears in uploaded photos before creating a journal musing.
